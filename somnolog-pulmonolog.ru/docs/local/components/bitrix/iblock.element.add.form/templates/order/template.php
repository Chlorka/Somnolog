...
<form class="form_send"
      ...
      data-ajax="ajax_order"
      data-result="ww-order_res"
    >
	<?=bitrix_sessid_post()?>
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
        ...
        <?if($propertyID != 'PREVIEW_TEXT'):?>
				<?if($propertyID == 'NAME' ) {?>
                    <div class="ww-grid_tr">
                <?}
                elseif($propertyID == '1') {?>
                    </div>
                <?}?>
                    <div class="ww-grid <?if($propertyID == '5' || $propertyID == 'NAME' ):?>ww-row2<?endif;?>">
                    ...
                    ?>
                    <?/*if($INPUT_TYPE != 'T'):?>
                        <label><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>(ОБЯЗАТЕЛЬНО)<?endif?></label>
                    <?endif;*/?>
                    <?
                    switch ($INPUT_TYPE):
							case "USER_TYPE":
	...
	<?if($propertyID=='PREVIEW_TEXT'):?>
		...
		<?endif?>
    <div class="ww-grid">
        <input name="only"  type="hidden" value="order">
...
<p class="order-text">Мы перезвоним вам в течении часа рабочего времени и согласуем заказ</p>
</form>
</div>
